import { Pokemon, PokemonsResponse } from "@/app/_types/pokemonTypes";
...
export async function generateStaticParams() {
  ...
  const pokemons = data.results.map((pokemon) => ({
    slug: pokemon.name.toString(),
  }));

  return pokemons;
}

// *INFO: generate static page from a dynamic page
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const { slug } = await params;
    const { id, name } = await getPokemon(slug);
